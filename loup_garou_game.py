...
def return_dict_from_json_or_fix(message_json):
    ...
    if (list_of_responses != []):
        ...
        global timerOn
        if datetime.datetime.now() - timerOn > datetime.timedelta(seconds=10):
...
class Game:
    ...
    def __init__(self, player_count, discussion_depth, render_markdown, real_player_name=""):
    ...
    def initialize_game(self):
        ...
        if (self.real_player_name != ""):
            ...
            if (choice.lower() == 'random'):
                ...
                self.players.append(Player(self.real_player_name, 0, self.get_other_players(self.player_names.index(self.real_player_name), self.player_names), "Villager", card_list))
                for i in range(len(self.player_names)):
                    if (self.player_names != self.real_player_name):
                        self.players.append(Player(self.player_names[i], i+1, self.get_other_players(i, self.player_names), alloted_cards[i], card_list))
                alloted_cards.insert(0, "Villager")
            elif (choice.lower() == 'werewolf'):
                alloted_cards.remove("Werewolf")
                self.players.append(Player(self.real_player_name, 1, self.get_other_players(self.player_names.index(self.real_player_name), self.player_names), "Werewolf", card_list))
                for i in range(len(self.player_names)):
                    if (self.player_names != self.real_player_name):
                        self.players.append(Player(self.player_names[i], i+1, self.get_other_players(i, self.player_names), alloted_cards[i], card_list))
                ...
                self.players.append(Player(self.real_player_name, 1, self.get_other_players(self.player_names.index(self.real_player_name), self.player_names), "Seer", card_list))
                for i in range(len(self.player_names)):
                    if (self.player_names != self.real_player_name):
                        self.players.append(Player(self.player_names[i], i+1, self.get_other_players(i, self.player_names), alloted_cards[i], card_list))
    ...
    def day(self):
        ...
        while discussion_count < self.discussion_depth:
            ...
            if (player.player_name.lower() == self.real_player_name.lower()):
                is_targeting = input("Would you like to say something directly to one of the players? Enter 'Yes' or 'No': ")
                if (is_targeting.lower() == 'yes'):
                    target_player = input(f"To speak directly to player, enter one of the following names {self.player_names}: ")
    ...
    def vote(self):
        ...
        for player in self.players:
            ...
            timerOn = datetime.datetime.now()
            if (player.player_name.lower() == self.real_player_name.lower()):
                voted_player = input(f"Vote to exclude the player. Enter one of the following names {self.player_names}: " )
...
@click.option('--discussion-depth', type=int, default=5, help='Number of discussion rounds')
...
def play_game(player_count, discussion_depth, render_markdown):
    input_text = ""
    while (input_text != 'yes' and input_text != 'no'):
        input_text = input("Would you participe in Werewolf game board? Enter 'Yes' or 'No': ").lower()
    if (input_text == 'yes'):
        real_player_name = input("What's your name? ")
        game = Game(player_count=player_count, discussion_depth=discussion_depth, render_markdown=render_markdown, real_player_name=real_player_name)
    else:
